USE [GTDB2_devel]
GO
/****** Object:  StoredProcedure [dbo].[p_BulkQCGetGenotypesFromMarkerSelection]    Script Date: 11/16/2009 13:32:40 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE PROCEDURE [dbo].[p_BulkQCGetGenotypesFromMarkerSelection](@result_plate_id INTEGER, @file_path VARCHAR(255), @format_file_path VARCHAR(255))
AS
BEGIN
SET NOCOUNT ON
...
BEGIN
	...
	+ 'FIRSTROW = 2, FORMATFILE = ''' + @format_file_path + ''') AS gtf INNER JOIN '
	+ 'marker m ON (m.marker_id = gtf.marker_id) WHERE m.identifier IN '
