USE [GTDB2_devel]
GO
/****** Object:  StoredProcedure [dbo].[p_UpdateTubeAliquots]    Script Date: 11/20/2009 16:26:53 ******/
SET ANSI_NULLS OFF
GO
SET QUOTED_IDENTIFIER OFF
GO
ALTER PROCEDURE [dbo].[p_UpdateTubeAliquots](
	@ids VARCHAR(1024),
	...
	@update_sample_dilute_factor bit,
	@sample_dilute_factor varchar(50) = null,
	@update_volume bit,
...
if 	@update_fragment_length = 0 and @update_fragment_length_device = 0 and
	...
	@update_comment = 0 AND @update_concentration_device = 0 and @update_state = 0 and
	@update_sample_dilute_factor = 0 and @update_volume = 0
return
...
if @update_sample_dilute_factor = 1
	set @cmd = @cmd + 'sample_dilute_factor = ' + isnull(@sample_dilute_factor, 'null') + ','
if @update_volume = 1
	set @cmd = @cmd + 'volume = ' + isnull(@volume, 'null') + ','
-- Remove the last comma
set @cmd = substring(@cmd, 1, len(@cmd) - 1)
set @cmd = @cmd + ' where tube_aliquot_id in (' + @ids + ')'
...
IF @@ERROR <> 0
BEGIN
	RAISERROR('Failed to update tube_aliquot with ids: %d', 15, 1, @ids)
	RETURN
END
