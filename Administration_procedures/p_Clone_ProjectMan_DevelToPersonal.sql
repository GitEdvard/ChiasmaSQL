...
create procedure p_Clone_ProjectMan_DevelToPersonal @version int = null, @latest_release bit = 1, @latest_merge bit = 0
...
if @latest_merge = 1 begin
	set @backup_file = '''D:\Proc_references\Devel_backups\Latest merge\ProjectMan_devel_backup.bak'''
end
if @latest_release = 1 begin
	set @backup_file = '''D:\Proc_references\Devel_backups\Latest release\ProjectMan_devel_backup.bak'''
end

-- Find 'personal' devel db name for current user
select @devel_db_name = devel_db_name from map_devel_table where login = SYSTEM_USER and operation_db_name = 'ProjectMan'
