...
create procedure p_admin_Clone_Projman_DevelLatestRelease(
...
select @devel_db_name = devel_db_name from map_devel_table where login = @devel_login and operation_db_name = 'ProjectMan'

if isnull(@devel_db_name, '###') = '###' begin
	raiserror('Could not find devel db name for database ProjectMan (table map_devel_table in Administration)', 15,1)
end
...
set @sql = @sql + 
'restore database ' + @devel_db_name + ' from disk = ''D:\Proc_references\Devel_backups\Latest release\projectman_devel_backup.bak''
with move ''ProjectMan_devel'' to ''D:\SQLUserDBData\' + @devel_db_name + '.MDF'',
move ''ProjectMan_devel_log''to ''D:\SQLUserDBTransLogs\' + @devel_db_name + '.ldf'',
replace, recovery

alter database ' + @devel_db_name + '  modify file (name = ProjectMan_devel, newname = ' + @devel_db_name + '_Data)
alter database ' + @devel_db_name + ' modify file (name = ProjectMan_devel_log, newname = ' + @devel_db_name + '_Log)
