...
alter procedure p_admin_CloneGTDB2DevelLatestRelease(
...
if isnull(@devel_db_name, '###') = '###' begin
	raiserror('Could not find devel db name for database GTDB2 (table map_devel_table in Administration)', 15,1)
end
...
alter database ' + @devel_db_name + '  modify file (name = GTDB2_devel_Data, newname = ' + @devel_db_name + '_Data)
alter database ' + @devel_db_name + ' modify file (name = GTDB2_devel_Log, newname = ' + @devel_db_name + '_Log)
