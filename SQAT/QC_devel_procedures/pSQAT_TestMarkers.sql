USE [QC_devel]
GO
/****** Object:  StoredProcedure [dbo].[pSQAT_TestMarkers]    Script Date: 11/20/2009 16:46:04 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[pSQAT_TestMarkers](@Initiate BIT,
									@SelCmd VARCHAR(3000),
									@ItemMode VARCHAR(10),
...
DECLARE @selected_experiment TABLE(
				...
				allele_variant  CHAR(3) NOT NULL
				)
...
DECLARE @control_individual table(
			individual_id int,
...
-- Get a list of consensus genotypes according to control individuals (not according to samples)
-- Only used in 'sample' mode
DECLARE @consensus_genotype_for_cindv table(
			individual_id int not null,
			experiment_id int not null,
			genotype_id int null,
			alleles char(3) null,
			primary key (individual_id, experiment_id)
		)
...
BEGIN
	INSERT INTO @selected_experiment (experiment_id, identifier, chrom_name, allele_variant)
	SELECT a.assay_id, a.identifier, m.chrom_name, m.allele_variant
	FROM dbo.assay a
...
BEGIN
	INSERT INTO @selected_experiment (experiment_id, identifier, chrom_name, allele_variant)
	SELECT m.marker_id, m.identifier, m.chrom_name, m.allele_variant
	FROM dbo.marker m
...
CASE WHEN (COUNT(DISTINCT alleles) > 1) OR (@demandDuplicates = 1 AND COUNT(alleles) < 2) 
	THEN NULL 
	ELSE MAX(genotype_id) END AS consensus_genotype_id,
...
--select * from @consensus_genotype

--insert into #myconsensus_genotype
--(item_id, experiment_id, genotype_id, alleles, duplicate_tested)
--select item_id, experiment_id, genotype_id, alleles, duplicate_tested
--from @consensus_genotype

--------------------------------- Inheritance test ---------------------------------------------------------------

-- Report based on samples (not individuals)
IF @doInheritanceTest = 1
BEGIN
	IF @ItemMode = 'SAMPLE' and @doInheritanceTest = 1
	BEGIN
		-- For sample-based reports, do inheritance test on control samples only
		-- First, get a table of all involved individuals and get concensus genotypes for them
		-- Do inheritance test for the individuals

		-- @inheritance_test (table) is filled two times, first time (here) with item_id refering to individuals
		-- at last step, it is emptied and filled again, refering to samples instead
		
		-- get a list of control indivuduals in the selected set
		insert into @control_individual
		(individual_id, identifier, father_id, mother_id, sex_id)
		select distinct i.individual_id, i.identifier, i.father_id, i.mother_id, i.sex_id 
		from @inh_control_item ici 
			inner join sample s on s.sample_id = ici.item_id
			inner join individual i on i.individual_id = s.individual_id

		INSERT INTO @consensus_genotype_for_cindv (individual_id, experiment_id, genotype_id)
		SELECT s.individual_id, experiment_id,
		CASE WHEN (COUNT(DISTINCT alleles) > 1) THEN NULL ELSE MAX(genotype_id) END AS consensus_genotype_id
		FROM #result r
			inner join @inh_control_item ici on ici.item_id = r.item_id
			inner join sample s on s.sample_id = ici.item_id
		WHERE status_id IN (SELECT status_id FROM @valid_status) AND NOT alleles = 'N/A'
		GROUP BY s.individual_id, experiment_id


		-- Fill out the alleles of the consensus genotypes so that they can be used easily later.
		UPDATE cgfi
		SET alleles = r.alleles
		FROM @consensus_genotype_for_cindv cgfi
			inner join #result r on cgfi.genotype_id = r.genotype_id
		where not r.alleles = 'N/A' and isnull(cgfi.genotype_id, -1) > -1
		...
		SELECT cgfi.individual_id, cgfi.experiment_id, cgfi.alleles, ci.sex_id, se.chrom_name, ci.father_id, ci.mother_id, cgf.alleles, cgm.alleles, 0
		FROM @consensus_genotype_for_cindv cgfi
		INNER JOIN @selected_experiment se ON se.experiment_id = cgfi.experiment_id
		INNER JOIN @control_individual ci ON ci.individual_id = cgfi.individual_id
		LEFT OUTER JOIN @consensus_genotype_for_cindv cgf ON (cgf.individual_id = ci.father_id AND cgf.experiment_id = cgfi.experiment_id)
		LEFT OUTER JOIN @consensus_genotype_for_cindv cgm ON (cgm.individual_id = ci.mother_id AND cgm.experiment_id = cgfi.experiment_id)	
		WHERE (NOT cgfi.alleles IS NULL)  -- Cannot test if the child is NULL 
		AND NOT (cgf.alleles IS NULL AND cgm.alleles IS NULL) -- Cannot test if both parents are NULL
		AND NOT (ISNULL(se.chrom_name, '') = 'X' AND cgm.alleles IS NULL AND ci.sex_id = @maleSexId AND @detectX = 1)  -- Cannot test on human X chrom if child is male and mother is NULL.			
...
--insert into #myinheritance_case
--select * from #inheritance_case
	...
	FROM #inheritance_case
	WHERE NOT father_id IS NULL and not father_alleles is null
	UNION
	SELECT mother_id, experiment_id, 1
	FROM #inheritance_case
	WHERE NOT mother_id IS NULL and not mother_alleles is null
	...
	IF @ItemMode = 'SAMPLE'
	BEGIN
		-- At this point, @inheritance_test.item_id refers to individual
		-- Make it refer to sample instead

		DECLARE @inh_test_sample TABLE( 
            item_id INTEGER NOT NULL, 
            experiment_id INTEGER NOT NULL, 
            success BIT NOT NULL, 
            PRIMARY KEY (item_id, experiment_id)) 

		insert into @inh_test_sample
		(item_id, experiment_id, success)
		select s.sample_id, it.experiment_id, it.success
		from @inheritance_test it 
			inner join sample s on s.individual_id = it.item_id
			inner join @selected_item si on si.item_id = s.sample_id

		delete from @inheritance_test

		insert into @inheritance_test
		(item_id, experiment_id, success)
		select item_id, experiment_id, success 
		from @inh_test_sample

		delete from @inh_test_sample

	END

END -- if do inheritance test
...
LEFT OUTER JOIN @zygosity_count_preparation xx ON 
	( xx.experiment_id = se.experiment_id
	  ...
	  AND SUBSTRING(se.allele_variant, 1, 1) = SUBSTRING(xx.alleles, 1, 1)
	)
...
LEFT OUTER JOIN @zygosity_count_preparation yy ON 
	( yy.experiment_id = se.experiment_id
	  ...
	  AND SUBSTRING(se.allele_variant, 3, 1) = SUBSTRING(yy.alleles, 1, 1)
	)
...
--
IF @ItemMode = 'SUBJECT' -- individual
BEGIN
...
ELSE -- samples
BEGIN
	-- @inh_control_stat => total and failed number of inheritance tests grouped by marker/assay
	-- in sample mode, count by individual
	INSERT INTO @inh_control_stat (experiment_id, failed_items, tested_items)
	SELECT se.experiment_id, ISNULL(failed.failed_items, 0), ISNULL(tested.tested_items, 0)
	FROM @selected_experiment se
	LEFT OUTER JOIN
		(SELECT experiment_id, COUNT(distinct s.individual_id) AS failed_items
		FROM @inheritance_test it
			inner join sample s on s.sample_id = it.item_id
		WHERE success = 0 AND item_id IN (SELECT item_id FROM @inh_control_item)
		GROUP BY experiment_id) failed ON failed.experiment_id = se.experiment_id
	LEFT OUTER JOIN
		(SELECT experiment_id, COUNT(distinct s.individual_id) AS tested_items
		FROM @inheritance_test it
			inner join sample s on s.sample_id = it.item_id
		WHERE item_id IN (SELECT item_id FROM @inh_control_item)
		GROUP BY experiment_id) tested ON tested.experiment_id = se.experiment_id
...
--insert into #myinh_test(
--item_id, experiment_id, success)
--select item_id, experiment_id, success
--from @inheritance_test
--
--insert into #myinh_control_stat(
--experiment_id, failed_items, tested_items
--)
--select experiment_id, failed_items, tested_items 
--from @inh_control_stat
